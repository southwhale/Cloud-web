...
CloudMain.Time={
    elm:function(){
        this.elm=$(".CloudTimeClock *")
    },
    text:function(){
        this.text=$(".CloudTimeText *")
    },
    clock:function(){
        this.clock=this.elm[0];
    },
    ...
    show:function () {
        ...
        CloudMain.Time.text[0].innerHTML =hHour + ":" + $.String.zeroize(mMinute)+'<span class="CloudTimeTag">'+tag+'</span><span>'+$.String.zeroize(Ssecond)+'</span>';
        CloudMain.Time.elm[1].style.webkitTransform="rotate("+(iHour*360/12)+"deg)";
        CloudMain.Time.elm[2].style.webkitTransform="rotate("+(iMin*360/60)+"deg)";
        CloudMain.Time.elm[3].style.webkitTransform="rotate("+(iSec*360/60)+"deg)";
    ...
    init:function () {
        ...
        this.show();
        for (var i = 0; i < 12; i ++){
            CloudMain.CloudStyle.innerHTML+=".CloudTimeClock ul li:nth-of-type("+(i+1)+"){-webkit-transform: rotate("+i*30+"deg);}";
            this.clock.innerHTML +="<li></li>";
        }
        CloudMain.Time.text[3].innerHTML=$.Time.now('Y年m月d日')+' '+CloudMain.Time.week[new Date().getDay()];
...
CloudMain.Func={
    ...
    modern:function () {
        return [{
            ...
            'img': './public/img/bar/'+CloudMain.User.theme+'/couple.png',
            'name': '问题反馈',
            ...
            (function (i) {
                ...
                $.RGBaster.colors(_this.modern()[i].img, {
                    ...
                    success: function(payload) {
                        a.style.background=payload.dominant;
                    }
                });
            })(i)
        }
    ...
    position:function () {
        var _this=this;
        var l = parseInt(this.panel.offsetWidth/(this.elm[0].offsetLeft+this.elm[0].offsetWidth))-1;
